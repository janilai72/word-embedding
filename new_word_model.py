...
class TrieNode(object):
    ...
    """
    def __init__(self, node, data=None, PMI_limit=3):
        """
        初始函数，data为外部词频数据集
        :param node:
        :param data:
    ...
    def search_bi(self,PMI_limit):
        ...
        for child in node.child:
            for ch in child.child:
                if ch.word_finish is True:
                    # 互信息值越大，说明 a,b 两个词相关性越大 log('雪落山庄')-log(2gram-total)-log('雪落'/1gram-total)-log('山庄'/1gram-total)
    ...
    def find_word(self, N,PMI_limit=3):
        # 通过搜索得到互信息
        # 例如: dict{ "a_b": (PMI, 出现概率), .. }
        bi = self.search_bi(PMI_limit)
        # 通过搜索得到左右熵
